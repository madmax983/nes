...
**Action:** Extract the complex `RtaManager` initialization into a private `setup_rta_manager(rta_config, rom_hash)` helper function to flatten `run()` and isolate the profile decision tree. Ensure code coverage is maintained by writing explicit test cases for the extracted function.
