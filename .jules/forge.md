...
**[Refactoring duplicate time machine & metrics reset boilerplate in AppAction matches]**
**Learning:** In `crates/nes-desktop/src/main.rs`, within `execute_app_action()`, the `AppAction::OpenRom`, `AppAction::LoadSlot`, and `AppAction::Reset` arms all contained an identical block of code to reset the rewind state, time machine, and performance metrics. This boilerplate made the match arms long and hard to read, creating a "God Function" smell for `execute_app_action()`. Additionally, both `OpenRom` and `LoadSlot` cleared the audio buffer, but `Reset` did not.
**Action:** Extracted this repeated logic into a helper function `reset_time_machine_and_metrics(ctx: &mut AppContext<'_>)`. This reduced boilerplate, simplified the match arms, and ensured consistent state resetting across all three actions (including clearing the audio buffer on Reset).
