...
**Context Struct Extraction for Action Dispatchers**
...
**[Flattening `if` blocks with `match` in classify_keyboard_input]**
**Learning:** `classify_keyboard_input` had a series of `if pressed && key == ...` conditions. Using `match` with guard clauses and tuple destructuring greatly improves readability by flattening the logic and grouping conditions for specific states.
**Action:** Converted sequential `if` checks into a single `match (key, pressed)` block.

**[Simplifying set_overlay_open call signatures]**
**Learning:** `set_overlay_open` took 6 arguments, 5 of which were fields from `AppContext`. When called repeatedly, this creates significant boilerplate.
**Action:** Updated `set_overlay_open` to take `ctx: &mut AppContext<'_>` instead of individual fields, drastically reducing call site bloat.
