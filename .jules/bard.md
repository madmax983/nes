...
## 2025-06-18 - Missing README setup instruction
**Confusion:** The README instructed users to run commands using `--config ./nes.toml`, but that file doesn't exist by default, causing a file not found error (`os error 2`).
**Clarification:** Added an explicit `cp nes.example.toml nes.toml` step in the README right before the launch commands so users have a configuration file to point to.
