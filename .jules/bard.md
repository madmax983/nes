...
## 2024-05-10 - Do Not Add Boilerplate to Simple Getters
**Confusion:** The instructions state to document the "Missing Link" (public functions with no `///` comments). This led to automatically adding noisy doctests to trivial getters (like `is_paused` or `total_cycles`) which clutter the code.
**Clarification:** The "BARD AVOIDS (Noise)" rule explicitly states to avoid "Getters" docs and useless noise. Focus on complex logic, core abstractions, and explaining *why* a function exists (like `mapper_delta` optimizing rewind functionality). Always check if the documentation genuinely helps human understanding before writing it.
