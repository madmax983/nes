...
## 2025-06-25 - Finding and Fixing Missing Docs
**Confusion:** Sometimes missing documentation doesn't show up in `cargo check` or `cargo test` if the `#![warn(missing_docs)]` lint isn't enabled globally or per-file.
**Clarification:** You can use `RUSTDOCFLAGS="-D missing_docs -D rustdoc::missing_crate_level_docs" cargo doc --no-deps --workspace` to strictly enforce and expose all missing documentation errors across the entire workspace.
