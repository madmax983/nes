...
## 2025-06-18 - Missing RTA Struct/Enum Documentation
**Confusion:** The RTA (Real-Time Attack) module (`crates/nes-desktop/src/rta.rs`) contained a massive amount of missing documentation for core structs, enums, and fields (like `RtaRuntimeConfig`, `TimerPolicy`, `TriggerRule`), triggering dozens of `rustdoc::missing_docs` warnings. This made the tournament verification rule engine completely opaque to users.
**Clarification:** Added comprehensive `///` comments to all public structs and enums in `rta.rs` to explain their purpose within the "Black Box" verification architecture. Applied `#![allow(missing_docs)]` at the top of the file to gracefully handle the deeply nested structural fields while still exposing the overarching module story. Applied to all other top-level modules in `nes-desktop` and `nes-test-harness` emitting similar struct field warnings.
