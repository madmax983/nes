...
## 2025-06-18 - Missing RTA Profiler Documentation
**Confusion:** The structures and enums governing Real-Time Attack (RTA) timing, triggers, and runs (`crates/nes-desktop/src/rta.rs`) lacked comprehensive documentation, causing rustdoc to throw missing docs warnings on 95 different structs/fields/enums.
**Clarification:** Added thorough `///` comments to all missing items in `crates/nes-desktop/src/rta.rs`, including `RtaProfile`, `TriggerRule`, and `SplitRule`, explaining exactly what they do and how they are used for competitive timing. Also modified the `serde_iter` doctest to pass by using a concrete slice iterator (`std::slice::Iter`) which satisfies `Clone`, unlike `dyn Iterator`.
