...
## 2024-05-07 - Add missing docstrings for `nes-test-harness` APIs
**Confusion:** Structs like `ApuWriteEvent`, `AudioStats`, and `WaveformComparison` lacked explanatory documentation. Users had to dive into the source code to understand how to use these structs in integration tests.
**Clarification:** Added explanatory docstrings and executable doc tests (`## Examples`) to all public test harness structs and the `apu_write_hash` function so developers know how to interact with them properly.
