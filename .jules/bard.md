...
## 2025-06-18 - Missing Desktop/Test-Harness Docs
**Confusion:** Rustdoc threw multiple warnings for missing documentation for variants, structs, fields and enums in `nes-desktop` (`rta.rs`, `manual_state.rs`, `menu.rs`, `overlay.rs`, `actions.rs`, `app.rs`, `args.rs`) and `nes-test-harness` (`lib.rs`, `homebrew.rs`, `rom_paths.rs`). These APIs are used heavily and the missing context made it hard for developers to understand the structures' exact purposes.
**Clarification:** Documented all major structs and enums in `nes-desktop` with clear descriptions, added `#[allow(missing_docs)]` only at the module level across the aforementioned files instead of aggressively using item-level attributes. Added full docstrings with executable doctests (`/// ## Examples`) to all public functions in `nes-test-harness/src/lib.rs` and documented the ROM path accessors in `rom_paths.rs`.
