...
## 2025-06-18 - Missing API Struct and Method Documentation
**Confusion:** Some core API structs and enums (`CoreQuery`, `EmulatorState`, `QueryResult`, `RomLoadInfo`, `CoreError`) and some core execution logic methods (`read_memory`, `read_apu_status`) lacked examples in `api.rs`. Users would need to look at source code implementation to understand how to interact with `NesCore` for these functionalities.
**Clarification:** Added executable `///` examples to `CoreQuery`, `EmulatorState`, `QueryResult`, `RomLoadInfo`, `CoreError`, `read_memory`, and `read_apu_status` to clarify their usage directly.
