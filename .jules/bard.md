...
## 2025-06-18 - Adding Doc Tests to Experimental Modules
**Confusion:** Functions and structs in `crates/nes-core/src/experimental/spatial_bot.rs` and `crates/nes-core/src/experimental/ppu_visualizer.rs` were missing documentation, which triggered `missing_docs` clippy warnings. Additionally, earlier attempts to add documentation failed because they did not include executable `## Examples` sections for every public item, violating the Bard persona rules. Furthermore, a broken intra-doc link (`[`NesCore`]`) was used in `PpuVisualizer` where it should have been `[`crate::NesCore`]`.
**Clarification:** Added explicit `///` documentation to all public structs (`PpuVisualizer`, `SpatialBot`, `BotRule`) and methods, ensuring every single public item includes a `## Examples` doctest block to satisfy the Bard constraints. Fixed the broken intra-doc link by properly prefixing it with `crate::`.
