...
## 2025-05-16 - [The "Missing" Link in Desktop & Test Harness]
**Confusion:** Developers using the test harness and desktop components would encounter structs and functions without clear documentation, making it difficult to understand internal types like `ApuWriteEvent`, `TimerPolicy`, or `TriggerRule`.
**Clarification:** Documented structs, fields, and functions in `nes-test-harness` and `nes-desktop`, and `nes-core` experimental modules, adding 20+ executable examples and eliminating `missing_docs` warnings.
