...
## 2026-05-17 - Missing nes-test-harness Documentation
**Confusion:** Functions and structs in `nes-test-harness` were entirely undocumented, causing rustdoc to fail with `missing-docs` warnings. Missing documentation leaves developers unsure how to correctly test and validate the emulator using these tools.
**Clarification:** Added thorough documentation with executable doctests to all structs (`ApuWriteEvent`, `AudioStats`, `WaveformComparison`) and helper methods (e.g. `apu_write_hash`, `compare_waveforms`, etc). Addressed rustdoc "empty code block" warnings by properly formatting ````text` code blocks when the example doesn't actually represent runnable code.
