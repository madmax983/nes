...
## 2025-06-25 - [Add Missing Examples for `nes-test-harness`]
**Confusion:** The structs `ApuWriteEvent`, `AudioStats`, and `WaveformComparison` were undocumented, as well as functions `apu_write_hash`, `waveform_hash`, `audio_stats`, `rms_envelope`, `compare_waveforms`, `fft_log_mag_db`, `detect_mapper_id`, `mapper_supported_by_core`, `default_homebrew_rom_path`, `smb_rom_path`, `nestest_rom_path`, `blargg_cpu_rom_path`, `bbbradsmith_audio_suite_rom_paths`, and `bbbradsmith_audio_golden_dir_path`.
**Clarification:** Added thorough `///` comments containing `## Examples` and `## Panics` sections to all these public items in `nes-test-harness`, providing executable context and removing warnings when compiling docs with `#![warn(missing_docs)]`.
