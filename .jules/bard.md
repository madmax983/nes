...
## 2025-06-18 - Missing documentation for nes-test-harness structs and functions
**Confusion:** Several structs (like `ApuWriteEvent`, `AudioStats`, `WaveformComparison`) and core testing utility functions in `nes-test-harness` were entirely missing documentation, causing warnings under `RUSTDOCFLAGS="-D missing_docs"`. Users had to read the source to understand what data these structs held and what the functions did.
**Clarification:** Added thorough `///` doc comments for all previously undocumented structs and functions in `nes-test-harness` to explain their purpose and provide clear usage guidelines.
