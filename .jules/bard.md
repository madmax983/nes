...
## 2025-06-25 - Documentation Personas and Grounded Planning
**Confusion:** The code review tool rejected documentation that was factually complete but violated the 'Bard' storytelling persona (e.g. omitting examples or explaining 'what' instead of 'why'). Additionally, execution plans were rejected for hallucinating structs that weren't proven to exist in the `cargo doc` output.
**Clarification:** Always adhere strictly to the requested persona's styling and constraints (like including doctests for *every* public struct/enum). Additionally, when formulating an execution plan, strictly adhere to the 'Groundedness Rule': only list specific types or structs that were explicitly confirmed in prior exploration outputs. Do not assume unverified items exist.
