...
## 2025-06-25 - [Missing struct doc comments]
**Confusion:** Some experimental modules such as `SpatialBot` and `PpuVisualizer` were missing documentation for their structures and fields, leading to missing_docs warnings. In addition, an intra-doc link pointing to a private item `DOTS_PER_CPU_CYCLE` in `mapper/fme7.rs` caused a rustdoc warning.
**Clarification:** Added missing `///` struct and field comments for `SpatialBot`, `BotRule`, and `PpuVisualizer` including doctests where appropriate, and changed the `[`DOTS_PER_CPU_CYCLE`]` link to a simple inline code block `` `DOTS_PER_CPU_CYCLE` `` to resolve warnings.
