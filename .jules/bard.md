...
## 2025-06-18 - The Danger of Duplicate Docstrings

**Confusion:** I used automated Python string-replacement scripts (`replace(old, new)`) to rapidly inject `///` documentation blocks into multiple files (`rta.rs`, `manual_state.rs`, `menu.rs`, etc.). However, my initial search targets were too broad (e.g. matching just `pub struct Foo {` or `#[derive(...)]`), which inadvertently matched code that already had attributes like `#[serde(...)]` or competing docstrings. This caused catastrophic duplication of traits (`E0119 conflicting implementations`) because `replace` blindly stacked identical `#[derive]` blocks, and resulted in noisy, stacked docstrings in the output.

**Clarification:** When programmatically injecting documentation via scripts, the search string must capture the *entire, exact context* (including all existing macros, attributes, and visibility modifiers) to guarantee the replacement doesn't accidentally duplicate code. A better approach is to match the exact block and rewrite it entirely, ensuring `///` is placed idiomatically before any `#[derive(...)]` attributes, not after them.
