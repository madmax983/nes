...
## 2025-06-20 - Adding Doc Tests to Missing Test Harness APIs
**Confusion:** The integration test utilities and data structures in `nes-test-harness` were entirely undocumented. Functions like `apu_write_hash` or `waveform_hash` didn't explain *why* hashes were used instead of golden files, confusing developers writing integration tests.
**Clarification:** Added explicit documentation comments `///` to structures (`AudioStats`, `WaveformComparison`) and functions, focusing on explaining their deterministic testing role. Also added 13 executable `## Examples` block doc-tests, ensuring future examples will continue to compile via `cargo test`.
