...
## 2025-06-25 - Fixing Missing Documentation Warnings and Missing Examples
**Confusion:** The documentation generation was throwing warnings due to the use of an intra-doc link `[`DOTS_PER_CPU_CYCLE`]` to a private internal constant. Additionally, many public functions in Mapper implementations, such as `ColorDreams::from_prg_chr`, lacked executable examples showing how to instantiate them.
**Clarification:** Replaced the intra-doc link with standard backticks `` `DOTS_PER_CPU_CYCLE` `` to resolve the missing docs warning without exposing internal details. I also added a `## Examples` executable block to `ColorDreams::from_prg_chr` to show how to construct mappers properly.
