...
## 2024-05-19 - Documenting Utility Crates and Experimental Modules
**Confusion:** Functions in `nes-test-harness`, such as `compare_waveforms` and `detect_mapper_id`, as well as `SpatialBot` and `PpuVisualizer` in `nes-core/src/experimental`, lacked any documentation. This caused warnings during `cargo rustdoc` when run with strict linting (`-W missing_docs`), leaving developers to guess the context of these tools.
**Clarification:** Added `///` module-level and function-level documentation to all missing items in `nes-test-harness` and `nes-core/src/experimental`, complete with executable `## Examples` that demonstrate their usage and assertions. Also resolved a rustdoc warning regarding an intra-doc link to a private item `DOTS_PER_CPU_CYCLE` in `fme7.rs`.
## 2024-05-19 - Documenting Utility Crates and Experimental Modules
**Confusion:** Functions in `nes-test-harness`, such as `compare_waveforms` and `detect_mapper_id`, as well as `SpatialBot` and `PpuVisualizer` in `nes-core/src/experimental`, lacked any documentation. This caused warnings during `cargo rustdoc` when run with strict linting (`-W missing_docs`), leaving developers to guess the context of these tools.
**Clarification:** Added `///` module-level and function-level documentation to all missing items in `nes-test-harness` and `nes-core/src/experimental`, complete with executable `## Examples` that demonstrate their usage and assertions. Also resolved a rustdoc warning regarding an intra-doc link to a private item `DOTS_PER_CPU_CYCLE` in `fme7.rs`.
