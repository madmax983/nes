...
## 2024-06-02 - Missing documentation
**Confusion:** Some files in `nes-test-harness` and `nes-desktop` were lacking sufficient documentation which resulted in compilation warnings when using `-D missing_docs`.
**Clarification:** Added comprehensive and descriptive `///` doc strings to all structs, methods, enums, variants and constant variables according to Bard's philosophy to clear up the missing docs errors.
