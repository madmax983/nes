...
## 2025-06-18 - Missing Documentation for Testing Utilities
**Confusion:** Functions and structs inside `nes-test-harness` were missing documentation, which caused compilation failures when `RUSTDOCFLAGS="-D missing_docs"` was used.
**Clarification:** Added comprehensive documentation with `///` and executable `## Examples` doctests to structs like `ApuWriteEvent` and functions like `detect_mapper_id`, providing explicit use-cases.
