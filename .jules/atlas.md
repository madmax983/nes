...
**Tangle:** The `api.rs` file in `nes-core` was bloated and contained input event logic and structs (`Button`, `Player`, `ControllerState`, `ControllerPorts`) mixed alongside the core emulator event loop API. This violated domain boundaries and high cohesion.
**Blueprint:** Extracted the input types and port-reading functions into a dedicated `crates/nes-core/src/input.rs` internal module. Updated `lib.rs` to register the new module via `pub mod input;` and exported `Button` and `Player` directly from the crate, significantly reducing `api.rs` file size and separating input logic from core emulation loop boilerplate.
