...
**Extract Input Logic from main.rs (Duplicate mapping function)**
**Tangle:** The `map_virtual_keycode` function in `crates/nes-desktop/src/main.rs` was a duplicate of the one already extracted to `crates/nes-desktop/src/input.rs` in an earlier refactor, violating the DRY principle and introducing a code smell.
**Blueprint:** Removed the duplicate `map_virtual_keycode` definition and its associated unit test `map_virtual_keycode_maps_all_supported_keys` from `crates/nes-desktop/src/main.rs`. Updated the call-site in `track_keyboard_bits_for_key` to properly use the centralized `crate::input::map_virtual_keycode` function. This restores the single source of truth for key mappings.
