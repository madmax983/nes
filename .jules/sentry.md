...
## 2026-06-21 - PPU Live Updates
**Learning:** `unwrap()` on `VecDeque::pop_front` is safe if the loop condition uses `front().is_some_and(...)`.
**Action:** Replaced `unwrap()` with explicit `expect` and added tests for PPU live update handling.
