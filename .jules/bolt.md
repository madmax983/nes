...
**[Avoiding Allocation for Loop I/O buffers]
**Learning:** [When reading line-based formats in a loop (e.g., using `BufRead::read_line`), declaring `let mut line = String::new()` inside the loop causes a heap allocation for every iteration.]
**Action:** [Hoist the `String` allocation outside the loop and call `line.clear()` before each `read_line` to reuse the existing buffer.]
