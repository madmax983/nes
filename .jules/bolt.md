...
## YYYY-MM-DD - Refactor Vec to Box in PPU
**Learning:** `Vec<u8>` adds unnecessary capacity and growth semantics for a strictly-sized snapshot structure, while also allocating implicitly. Using `Box<[u8]>` instead of `Vec<u8>` or `[u8; N]` in Serde derives avoids trait boundary errors, because `Box<[u8]>` correctly serializes directly without a custom wrapper.
**Action:** Replace `Vec<u8>` with `Box<[u8]>` generated by `Box::from(slice)` whenever capturing snapshot slices to improve memory behavior and serialization performance.
