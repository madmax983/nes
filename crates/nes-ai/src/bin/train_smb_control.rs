...
fn run() -> Result<(), String> {
    ...
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!("{} train_smb_control <profile_toml> [episodes] [checkpoint_dir] [artifact_dir]", "Usage:".with(crossterm::style::Color::Cyan).bold());
        std::process::exit(0);
    }
    ...
    let summary = train_smb_control(&profile_cfg, &trainer_cfg, episodes)
        .map_err(|e| format!("{} Training failed: {e}", "Error:".with(Color::Red).bold()))?;
