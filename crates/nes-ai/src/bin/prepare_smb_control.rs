...
fn run() -> Result<(), String> {
    ...
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!("{} prepare_smb_control <rom_path> <bootstrap_tas_json> <output_snapshot>", "Usage:".with(Color::Cyan).bold());
        std::process::exit(0);
    }
    if args.len() != 4 {
        eprintln!("{} prepare_smb_control <rom_path> <bootstrap_tas_json> <output_snapshot>", "Usage:".with(Color::Cyan).bold());
        std::process::exit(1);
    }
