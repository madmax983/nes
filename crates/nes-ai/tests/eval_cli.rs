...
#[test]
fn eval_smb_control_with_help_flag_prints_usage_and_exits_success() {
    ...
    assert!(stdout.contains("Usage: eval_smb_control <profile_toml> <checkpoint_base> [episodes] [artifact_dir]"));
}
...
fn eval_smb_control_without_required_arguments_prints_usage_and_fails() {
    ...
    assert!(stderr.contains("Usage: eval_smb_control <profile_toml> <checkpoint_base> [episodes] [artifact_dir]"));
}
