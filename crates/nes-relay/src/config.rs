...
pub fn parse_args(args: Vec<String>) -> Result<RelayArgs, String> {
    ...
    while idx < args.len() {
        ...
        if parse_arg(&args, &mut idx, "--bind", |value: &str| {
        ...
        if parse_arg(&args, &mut idx, "--latency-ms", |value: &str| {
        ...
        if parse_arg(&args, &mut idx, "--jitter-ms", |value: &str| {
        ...
        if parse_arg(&args, &mut idx, "--loss-pct", |value: &str| {
        ...
        if parse_arg(&args, &mut idx, "--reorder-pct", |value: &str| {
