...
fn main() {
    ...
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!("{} nes-mcp-run-macro <rom_path> <script_path>", "Usage:".with(crossterm::style::Color::Cyan).bold());
        std::process::exit(0);
    }
    if args.len() != 3 {
        eprintln!("{} nes-mcp-run-macro <rom_path> <script_path>", "Usage:".with(crossterm::style::Color::Cyan).bold());
        std::process::exit(1);
    }
