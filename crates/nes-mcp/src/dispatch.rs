...
/// **⚡ Bolt Optimization:** Returns a `&str` instead of an owned `String` to avoid a
/// heap allocation on the hot path, especially eliminating the need to allocate `"default".to_owned()`
/// when the user omits the `slot` parameter.
