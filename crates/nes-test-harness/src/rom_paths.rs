...
/// Retrieves the Super Mario Bros ROM path from the `nes.toml` configuration.
///
/// Falls back to `desktop.rom_path` if `roms.smb` is not configured.
///
/// ## Panics
/// Panics if the path is missing from configuration or if the file does not exist on disk.
///
/// ## Examples
///
/// ```no_run
...
/// Retrieves the `nestest.nes` ROM path from the `nes.toml` configuration.
///
/// ## Panics
/// Panics if the path is missing from configuration or if the file does not exist on disk.
///
...
/// Retrieves the Blargg CPU behavior test ROM path from the `nes.toml` configuration.
///
/// ## Panics
/// Panics if the path is missing from configuration or if the file does not exist on disk.
///
...
/// Recursively discovers all NES ROM paths located within the configured bbbradsmith audio suite directory.
///
/// Returns an alphabetically sorted list of file paths ending in `.nes`.
///
/// ## Panics
/// Panics if the directory is missing from configuration, does not exist, or contains no `.nes` files.
///
/// ## Examples
///
/// ```no_run
...
/// Retrieves the output directory path for saving/loading golden audio `.pcm` waveforms
/// for the bbbradsmith audio suite.
///
/// ## Panics
/// Panics if the path is missing from configuration or if the directory does not exist.
///
/// ## Examples
///
/// ```no_run
