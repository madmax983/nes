...
/// Represents a single memory write operation targeting the Audio Processing Unit (APU) registers.
///
/// This struct is primarily used to trace and verify cycle-accurate sound events during emulation.
/// By collecting these events, integration tests can prove that the CPU execution timing and hardware
/// side-effects are completely deterministic.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ApuWriteEvent {
    /// The absolute CPU cycle timestamp when the write occurred since the emulator powered on.
    pub cpu_cycle: u64,
    /// The memory address that was written to. Typically bounded between `0x4000` and `0x4017`.
    pub addr: u16,
    /// The 8-bit data payload written to the register.
    pub value: u8,
}

/// A comprehensive statistical profile of a captured audio waveform.
///
/// This structure provides macro-level metrics about an audio stream, enabling tests to verify
/// properties like loudness (RMS), signal biasing (DC offset), and distortion (clipping).
/// It is usually generated by calling the [`audio_stats`] function on a raw PCM buffer.
///
/// ## Examples
///
/// ```
/// use nes_test_harness::{AudioStats, audio_stats};
///
/// let pulse_wave = [16000, 16000, -16000, -16000];
/// let profile: AudioStats = audio_stats(&pulse_wave);
/// assert_eq!(profile.peak, 16000);
/// assert_eq!(profile.dc_offset, 0.0);
/// ```
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct AudioStats {
    /// The total number of individual audio samples analyzed in the stream.
    pub sample_count: usize,
    /// Root Mean Square (RMS). Represents the perceived continuous loudness or "power" of the waveform.
    pub rms: f64,
    /// The maximum absolute amplitude magnitude found in the stream.
    pub peak: i16,
    /// The mean amplitude value across all samples. A non-zero value indicates the waveform is biased above or below silence.
    pub dc_offset: f64,
    /// The percentage (from `0.0` to `1.0`) of samples that hit or exceeded the maximum 16-bit integer boundaries, causing distortion.
    pub clipping_ratio: f64,
}

/// A comparative analysis between two distinct audio waveforms.
///
/// This struct acts as a similarity report, generated by [`compare_waveforms`]. It computes
/// mathematical relationships between two streams (such as a generated emulator audio track
/// versus a golden reference file) across time, amplitude, and frequency domains.
#[derive(Debug, Clone, PartialEq)]
pub struct WaveformComparison {
    /// The number of overlapping samples successfully evaluated between the two streams.
    pub samples_compared: usize,
    /// The Pearson correlation coefficient (`-1.0` to `1.0`). A value near `1.0` indicates identical wave shapes.
    pub correlation: f64,
    /// The ratio of the left stream's RMS power to the right stream's RMS power. Ideal is `1.0`.
    pub rms_ratio: f64,
    /// The average absolute difference in decibels across corresponding FFT frequency bins. Lower is better.
...
/// Hashes a chronological sequence of APU write events into a deterministic 64-bit identifier.
///
/// This provides a compact, easily comparable signature for an entire execution trace, allowing
/// test assertions to guarantee that millions of cycles produced the exact expected hardware writes.
///
/// ## Examples
///
/// ```
...
///
/// let writes = vec![
///     ApuWriteEvent { cpu_cycle: 12345, addr: 0x4000, value: 0x0F },
///     ApuWriteEvent { cpu_cycle: 12348, addr: 0x4001, value: 0x00 },
/// ];
/// let signature = apu_write_hash(&writes);
...
/// Hashes a sequence of audio samples into a deterministic 64-bit identifier.
///
/// Allows test cases to assert that generated PCM audio is precisely identical, bit-for-bit,
/// to previous successful runs, preventing subtle regressions in the audio synthesis engine.
///
/// ## Examples
///
/// ```
...
///
/// let silence = [0i16; 100];
/// let hash = waveform_hash(&silence);
/// // The hash of 100 zeros using this algorithm
/// assert_eq!(hash, 15202111851216601973);
/// ```
#[must_use]
...
/// Extracts macro-level mathematical properties from an array of audio samples.
///
/// Useful for validating test bounds, such as ensuring an output waveform is not entirely silent
/// or checking that it does not suffer from massive digital clipping.
///
/// ## Examples
///
/// ```
...
/// Calculates the changing RMS (loudness) power over time by grouping samples into sliding windows.
///
/// This provides a low-resolution "envelope" of the waveform, revealing macro-level volume
/// variations (like a sound fading out) rather than high-frequency oscillation data.
///
/// ## Examples
///
/// ```
...
///
/// let falling_wave = [32000, -32000, 1000, -1000];
/// let envelope = rms_envelope(&falling_wave, 2);
/// assert!(envelope[0] > envelope[1]); // First window is louder
/// ```
#[must_use]
...
/// Performs a rigorous time, amplitude, and frequency domain comparison between two audio streams.
///
/// Calculates correlation, RMS ratios, and FFT differences to determine how similar two streams are.
/// Crucial for "fuzzy" integration tests where emulator audio may not be bit-for-bit identical
/// to a reference stream (due to float rounding) but should sound identical to the human ear.
///
/// ## Examples
///
/// ```
...
///
/// let track_a = [0, 100, 200, 100, 0, -100, -200, -100];
/// let track_b = [1, 101, 201, 101, 1, -99, -199, -99]; // Slightly off
/// let diff = compare_waveforms(&track_a, &track_b, 8);
/// assert!(diff.correlation > 0.99); // Nearly identical shapes
/// ```
#[must_use]
...
/// Computes the Discrete Fourier Transform (DFT) magnitude spectrum of the given samples in decibels.
///
/// This function converts time-domain PCM samples into frequency-domain power bins,
/// allowing tests to assert that an audio stream contains energy at specific expected frequencies
/// (like checking if the square channel is actually playing an A4 note at 440Hz).
///
/// ## Examples
///
/// ```
...
///
/// let wave = [0, 16000, 0, -16000, 0, 16000, 0, -16000];
/// let bins = fft_log_mag_db(&wave, 8);
...
/// Parses the iNES header of a raw ROM byte array to determine its intended mapper ID.
///
/// Useful for skipping tests that require advanced or unsupported mappers.
///
/// ## Examples
///
/// ```
...
///
/// let mut smb_rom = vec![0_u8; 16];
/// smb_rom[0..4].copy_from_slice(b"NES\x1A");
/// smb_rom[6] = 0x00; // Mapper 0
/// smb_rom[7] = 0x00;
/// assert_eq!(detect_mapper_id(&smb_rom), Some(0));
/// ```
#[must_use]
...
/// Checks if the core emulator currently implements support for the specified mapper ID.
///
/// ## Examples
///
/// ```
