...
pub struct Cpu {
    ...
    status: Status,
    memory: [u8; 0x1_0000],
    /// ⚡ Bolt: Pre-allocated capacity to prevent multiple reallocations on the hot execution path.
    writes: Vec<CpuWrite>,
    /// ⚡ Bolt: Pre-allocated capacity to prevent multiple reallocations on the hot execution path.
...
impl Cpu {
    ...
    pub fn new(start_pc: u16) -> Self {
        Self {
            ...
            prg_writes: Vec::with_capacity(4),
            mmio_reads: RefCell::new(Vec::with_capacity(4)),
