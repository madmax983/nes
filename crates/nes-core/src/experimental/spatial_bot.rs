...
/// A condition-action pair that tells the bot how to react to a specific zone event.
///
/// This rule encapsulates the trigger (entering a zone) and the corresponding automated
/// input (pressing a button for a specific duration). It exists to allow configurable
/// and dynamic behaviors without hardcoding bot logic for every possible zone.
///
/// ## Examples
///
/// ```
...
#[derive(Debug, Clone)]
pub struct BotRule {
    /// The unique identifier of the zone that triggers this rule when an event occurs.
    pub zone_id: usize,
    /// The controller button to press when the rule is triggered.
    pub button: Button,
    /// The number of frames to hold the button down after the rule is triggered.
    pub duration_frames: u32,
}

/// A system that executes automated inputs based on spatial zone events.
///
/// `SpatialBot` bridges the gap between game state analysis (like entity tracking)
/// and automated controller inputs. By evaluating events generated by a `ZoneTracker`,
/// it can automatically trigger pre-configured button presses, effectively allowing
/// for programmatic gameplay responses based on visual/spatial conditions.
///
/// ## Examples
///
/// ```
...
/// // Bot is ready to have rules added and evaluate tracker events!
/// ```
#[cfg(feature = "nova")]
...
#[cfg(feature = "nova")]
impl SpatialBot {
    /// Creates a new, empty spatial bot with no rules or active presses.
    ///
    /// ## Examples
    ///
    /// ```
