...
/// A rule defining how the bot reacts to events in a specific zone.
#[cfg(feature = "nova")]
#[derive(Debug, Clone)]
pub struct BotRule {
    /// The ID of the zone to watch for events.
    pub zone_id: usize,
    /// The button to press when an event occurs.
    pub button: Button,
...
///
/// `SpatialBot` listens to events generated by a `ZoneTracker` and applies
/// configured rules to determine which buttons to press and for how long.
#[cfg(feature = "nova")]
...
impl SpatialBot {
    ...
    ///
    /// The bot starts with no rules and no active button presses.
    #[must_use]
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
