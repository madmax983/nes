...
impl NesCore {
    ...
    pub fn new() -> Self {
        Self {
            ...
            last_cpu_trace: None,
            // Performance optimization: Pre-allocate hot path buffers to prevent multiple heap reallocations when starting execution or capturing data.
            last_cpu_bus_trace: Vec::with_capacity(32),
            // Performance optimization: Pre-allocate hot path buffers to prevent multiple heap reallocations when starting execution or capturing data.
            scratch_writes: Vec::with_capacity(32),
            // Performance optimization: Pre-allocate hot path buffers to prevent multiple heap reallocations when starting execution or capturing data.
            scratch_mmio_reads: Vec::with_capacity(32),
        }
    }
...
mod tests_bolt {
    ...
    #[test]
    fn test_api_buffers_preallocated() {
        let core = NesCore::new();
        assert!(
            core.scratch_writes.capacity() >= 8,
            "scratch_writes vector must be pre-allocated"
        );
        assert!(
            core.scratch_mmio_reads.capacity() >= 8,
            "scratch_mmio_reads vector must be pre-allocated"
        );
        assert!(
            core.last_cpu_bus_trace.capacity() >= 8,
            "last_cpu_bus_trace vector must be pre-allocated"
        );
    }
}
