...
impl Ppu {
    ...
    fn apply_due_live_chr_updates(&mut self) {
        ...
        while let Some(update) = match self.pending_live_chr_updates.front() {
            Some(u) if u.due_cycle_in_frame <= current_cycle => self.pending_live_chr_updates.pop_front(),
            _ => None,
        } {
    ...
    fn apply_due_live_bg_updates(&mut self) {
        let current_cycle = self.cycle_in_frame();
        let mut applied = false;
        while let Some(update) = match self.pending_live_bg_updates.front() {
            Some(u) if u.due_cycle_in_frame <= current_cycle => self.pending_live_bg_updates.pop_front(),
            _ => None,
        } {
...
mod tests {
    ...
    #[test]
    fn should_not_panic_when_applying_due_live_chr_updates_with_empty_queue() {
        let mut ppu = Ppu::new();
