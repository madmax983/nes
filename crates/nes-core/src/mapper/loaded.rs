...
impl LoadedMapper {
    ...
    pub(crate) fn sync_chr_ram_from_ppu_window(&mut self, chr: &[u8; CHR_8K_BYTES]) {
        match self {
            Self::Mmc3(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Cnrom(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Gxrom(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::ColorDreams(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Camerica(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Namco108(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Fme7(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Mmc2(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Mmc4(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            Self::Mmc5(mapper) => mapper.sync_chr_ram_from_ppu_window(chr),
            _ => {}
        }
    }
