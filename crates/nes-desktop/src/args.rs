...
fn parse_netplay_arg(args: &[String], idx: &mut usize, parsed: &mut RuntimeArgs) -> Result<bool, String> {
    let arg = &args[*idx];
    if arg == "--netplay" {
...
fn parse_rta_arg(args: &[String], idx: &mut usize, parsed: &mut RuntimeArgs) -> Result<bool, String> {
    let arg = &args[*idx];
    if arg == "--rta" {
