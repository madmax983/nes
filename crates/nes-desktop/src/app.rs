...
/// Maps a platform UI input event to an actionable emulator or system command.
pub struct BridgeCommand {
    /// The core emulator command to be executed (e.g. `PressButton`).
    pub core: Command,
}

impl BridgeCommand {
    #[must_use]
    /// Returns a human readable name for the tool or command intended for debug logging.
...
#[must_use]
/// Maps a raw system keycode string and press state to a known UI or Emulator core command.
...
#[must_use]
/// Maps a raw system keycode string to an emulator controller `Button` enum if applicable.
...
#[must_use]
/// Maps a raw system keycode string to its bitwise representation for controller state tracking.
