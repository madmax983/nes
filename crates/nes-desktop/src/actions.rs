...
/// High-level actions triggered by the UI or OS events.
pub enum AppAction {
    /// Toggle the in-game overlay.
    ToggleOverlay,
    /// Resume emulation.
    Resume,
    /// Open a file picker to load a new ROM.
    OpenRom,
    /// Open the cheat code menu.
    OpenCheats,
    /// Save state to the given slot.
    SaveSlot(u8),
    /// Load state from the given slot.
    LoadSlot(u8),
