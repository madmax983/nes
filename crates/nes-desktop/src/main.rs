...
fn run() -> Result<(), String> {
    ...
    let mut rta_manager = runtime
        .rta
        .as_ref()
        .map(|config| setup_rta_manager(config, &session.rom_hash))
        .transpose()?;
        ...
        match select_profile(
            ...
            Err(err) => {
                if let Some(profile_id) = rta_config.profile_id_override.as_ref() {
                    ...
                    RtaProfile {
                        id: profile_id.clone(),
                        rom_hashes: vec![rom_hash.to_string()],
...
fn setup_rollback_engine(
    netplay: &crate::netplay::NetplayRuntimeConfig,
) -> Result<RollbackEngine, String> {
