...
fn run() -> Result<(), String> {
    ...
    let mut rta_manager = build_rta_manager(&runtime, &session)?;
...
fn build_rta_manager(
    runtime: &RuntimeConfig,
    session: &LoadedRomSession,
) -> Result<Option<RtaManager>, String> {
    let Some(rta_config) = runtime.rta.as_ref() else {
        return Ok(None);
    };
    ...
    let profile = if rta_config.calibrate {
        match select_profile(
            ...
            Err(err) => {
                let Some(profile_id) = rta_config.profile_id_override.as_ref() else {
                    return Err(format!(
