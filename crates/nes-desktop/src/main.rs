...
fn build_startup_table(
    ...
    add_row(
        "ROM Path",
        session.rom_path.display().to_string(),
        TableColor::Green,
    );
    add_row(
        "ROM Info",
    ...
    match step_mode {
        StepMode::Frame => {
            add_row("Step Mode", "frame".to_string(), TableColor::Green);
        }
        StepMode::CpuBudget(steps) => {
            add_row(
                "Step Mode",
    ...
    #[cfg(feature = "nova")]
    {
        if runtime.auto_player_enabled {
            add_row(
                "Nova",
                "Auto Player Chaos Fuzzing Enabled".to_string(),
                TableColor::White, // Note: The original didn't explicitly set a color, so White is a safe default or match existing
            );
        }
    }
