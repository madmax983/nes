...
fn run() -> Result<(), String> {
    ...
    startup_table.load_preset(UTF8_FULL);
    startup_table.set_header(vec![
        Cell::new("Component").fg(TableColor::Cyan),
        Cell::new("Status").fg(TableColor::White),
    ]);
    ...
    let audio_output = if runtime.audio_enabled {
        match AudioOutput::try_new() {
            Ok(output) => {
                add_startup_row("Audio", "Enabled", TableColor::Green);
                Some(output)
            }
            Err(err) => {
                add_startup_row("Audio", &format!("Disabled ({})", err), TableColor::Yellow);
                None
            }
        }
    } else {
        add_startup_row("Audio", "Disabled by config", TableColor::Yellow);
        None
    };

    if !startup_table.is_empty() {
        println!("\n{startup_table}");
    }
