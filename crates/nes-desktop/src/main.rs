...
fn dispatch_overlay_command(
    ...
    match command {
        ...
        OverlayCommand::ToggleCheat(index) => {
            ...
            let message = match ctx.session_cheats.toggle(index) {
                Ok(()) => apply_cheat_update(ctx.core, ctx.session_cheats, || {
                    let enabled = ctx
                        .session_cheats
                        .entries()
                        .get(index)
            ...
            ctx.overlay.set_status_message(message);
            ctx.window.request_redraw();
            false
        }
        OverlayCommand::RemoveCheat(index) => {
            let message = match ctx.session_cheats.remove(index) {
                Ok(removed) => apply_cheat_update(ctx.core, ctx.session_cheats, || {
                    format!("[cheat] removed {}", removed.raw_code)
                }),
                Err(err) => err.to_string(),
            };
            ctx.overlay.set_status_message(message);
        ...
        OverlayCommand::SubmitCheatCode(raw_code) => {
            let message = match ctx.session_cheats.add(&raw_code) {
                Ok(()) => apply_cheat_update(ctx.core, ctx.session_cheats, || {
                ...
                Err(err) => format!("Invalid cheat code '{}': {err}", raw_code.trim()),
            };
            ctx.overlay.set_status_message(message);
            ctx.window.request_redraw();
            false
...
#[inline]
fn apply_cheat_update<F>(core: &mut NesCore, cheats: &SessionCheats, success_msg: F) -> String
where
    F: FnOnce() -> String,
{
    match apply_session_cheats(core, cheats) {
        Ok(()) => success_msg(),
        Err(err) => err,
    }
}
