            ...
            WindowEventDecision::KeyboardInput { key, pressed } => {
                ...
                match classify_keyboard_input(key, pressed, mode) {
                    ...
                    KeyboardDecision::UpdateKeyboardBits { mask, pressed } => {
                        keyboard_bits = crate::input::update_button_bits(keyboard_bits, mask, pressed);
                    }
        ...
        Event::MainEventsCleared => {
                ...
                for player in 0..gamepad_bits.len() {
                    ...
                    if rollback.is_none()
                        ...
                        && let Err(err) = apply_gamepad_delta_commands(
                            ...
                            gamepad_bits[player],
                            next_gamepad_bits,
                            if crate::gamepad::is_player_two_slot(player) {
                                nes_core::Player::Two
                            } else {
                                nes_core::Player::One
                            },
            ...
            if let Some(rollback_engine) = rollback.as_mut() {
                ...
                let scheduled = rollback_engine
                    .schedule_local_input(crate::input::merge_local_input_bits(keyboard_bits, local_gamepad_bits));
...
mod tests {
    ...
    use super::{
        ...
        StepMode, WindowEventDecision, advance_core_for_host_frame, apply_overlay_keyboard_input,
        audio_queue_dropped, capture_path_for_frame, classify_window_event, connected_gamepad_ids,
        ...
        map_virtual_keycode, menu_action_enabled, overlay_input_requires_redraw,
        recommended_input_delay_frames, reconcile_core_pause_with_overlay, rom_picker_supported,
        ...
        should_update_input_delay, slot_action_for_hotkey, validate_action_allowed,
        write_frame_ppm,
    };
    use gilrs::GamepadId;
    ...
    fn desktop_loop_helper_primitives_cover_window_scale_and_player_flags() {
        ...
        assert!(!crate::gamepad::is_player_two_slot(0));
        assert!(crate::gamepad::is_player_two_slot(1));
        assert_eq!(
            crate::input::merge_local_input_bits(0b0000_0011, 0b0000_0101),
            0b0000_0111
        );
