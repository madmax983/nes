...
fn handle_save_slot(ctx: &mut AppContext<'_>, slot: u8) -> Result<bool, String> {
...
fn handle_load_slot(ctx: &mut AppContext<'_>, slot: u8) -> Result<bool, String> {
    if let Some(rta) = ctx.rta_manager.as_mut() {
        let _ = rta.mark_forbidden_action(
            ForbiddenAction::SaveLoad,
            ctx.frame_index,
...
fn execute_app_action(action: AppAction, ctx: &mut AppContext<'_>) -> Result<bool, String> {
    ...
    match action {
        ...
        AppAction::SaveSlot(slot) => handle_save_slot(ctx, slot),
        AppAction::LoadSlot(slot) => handle_load_slot(ctx, slot),
