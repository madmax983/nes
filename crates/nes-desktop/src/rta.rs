...
/// The default fallback directory for `.toml` speedrun profiles.
pub const DEFAULT_RTA_PROFILES_DIR: &str = "config/rta/profiles";
/// The default fallback directory where successful `.run.json` artifacts are saved.
pub const DEFAULT_RTA_RUNS_DIR: &str = "runs/rta";

#[derive(Debug, Clone)]
/// Runtime arguments passed to the application to bootstrap the RTA tracking environment.
pub struct RtaRuntimeConfig {
    /// A specific profile ID the user has forced the emulator to use, skipping ROM hash auto-detection.
    pub profile_id_override: Option<String>,
    /// The directory where the RTA manager will search for valid `.toml` profiles.
    pub profiles_dir: PathBuf,
    /// The directory where the RTA manager will write out `.run.json` artifacts.
    pub runs_dir: PathBuf,
    /// Whether the RTA manager should boot in calibration mode to reverse-engineer trigger addresses.
    pub calibrate: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Indicates whether a profile is fully validated for competition or is a rough draft.
pub enum ProfileStatus {
    /// A draft profile, typically generated by calibration mode. Auto-selection will ignore drafts.
    Draft,
    #[default]
    /// A published, competitive profile.
    Published,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Defines how the speedrun timer calculates elapsed time.
pub enum TimerClock {
    #[default]
    /// Uses real-world system wall time.
    Wall,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Policy that defines how the RTA manager should penalize or handle the emulator window losing operating system focus.
pub enum FocusLossPolicy {
    /// Automatically pauses the run timer until focus is regained.
    AutoPause,
    /// Instantly invalidates the run (strict competitive mode).
    Invalidate,
    #[default]
    /// Ignores focus loss entirely, allowing the run to continue in the background.
    Continue,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default, Hash)]
#[serde(rename_all = "snake_case")]
/// Specific emulator actions that provide an unfair advantage and can instantly invalidate a run.
pub enum ForbiddenAction {
    #[default]
    /// Using the emulator's rewind feature.
    Rewind,
    /// Manually loading a save state.
    SaveLoad,
    /// Stepping through emulation one frame at a time.
    FrameStep,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Specifies how many bytes of emulator memory should be read when evaluating a trigger.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The comparison operation used to evaluate if a memory value matches a target trigger condition.
pub enum TriggerOp {
    /// Evaluates to true if the memory value is strictly equal to the target value.
    #[default]
    Eq,
    /// Evaluates to true if the memory value is not equal to the target value.
    Ne,
    /// Evaluates to true if the memory value is strictly greater than the target value.
    Gt,
    /// Evaluates to true if the memory value is greater than or equal to the target value.
    Gte,
    /// Evaluates to true if the memory value is strictly less than the target value.
    Lt,
    /// Evaluates to true if the memory value is less than or equal to the target value.
    Lte,
    /// Evaluates to true if all bits specified in the target value are set in the memory value.
    BitSet,
    /// Evaluates to true if all bits specified in the target value are cleared (0) in the memory value.
    BitClear,
    /// Evaluates to true if the memory value has changed compared to the previous frame's value.
    Changed,
}

#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Dictates how the speedrun timer operates and responds to external events like losing window focus.
///
/// # Examples
///
/// ```rust
...
pub struct TimerPolicy {
    /// Determines whether the timer uses wall-clock time or emulation frame time.
    pub clock: TimerClock,
    /// Dictates what happens to the timer if the emulator window loses OS focus.
    pub focus_loss: FocusLossPolicy,
    /// If true, allows runners to manually start/split the timer using hotkeys even if auto-split triggers are defined.
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Defines the cardinal sins that will instantly invalidate a run, branding it as disqualified in the `.run.json`.
///
/// # Examples
///
/// ```rust
...
pub struct InvalidationPolicy {
    /// A list of forbidden emulator actions (e.g., Rewind, LoadState) that trigger instant run invalidation.
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Configures behavior around manual splitting and how split events are recorded.
///
/// # Examples
///
/// ```rust
...
///     append_only: true,
///     manual_hotkey: "F9".to_string(),
/// };
/// ```
pub struct SplitPolicy {
    /// If true, new splits only append to the run log. If false, splitting replaces previous splits (useful for single-segment tracking).
    pub append_only: bool,
    /// A string representation of the keyboard shortcut used to manually trigger a split event.
...
#[serde(default, deny_unknown_fields)]
/// Controls the generation of diagnostic and verification artifacts at the end of a run.
///
/// # Examples
///
/// ```rust
...
pub struct LoggingPolicy {
    /// If true, saves a frame-by-frame log of all controller inputs alongside the `.run.json` for perfect reconstruction or TAS validation.
    pub save_input_log: bool,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(default, deny_unknown_fields)]
/// A single condition that must be met in the emulator's memory space to trigger an RTA event (like a split).
///
/// Triggers work by reading a specified memory address every frame, optionally applying a width (e.g., u16),
/// and evaluating the result against a target `value` using the specified comparison `op`.
///
/// # Examples
///
/// ```rust
/// use nes_desktop::rta::{TriggerRule, TriggerOp, TriggerWidth};
/// let rule = TriggerRule {
///     address: 0x071A, // Current screen
///     width: TriggerWidth::U8,
///     op: TriggerOp::Eq,
///     value: 1, // Moving to screen 1
///     debounce_frames: 0,
///     require_consecutive: 1,
/// };
/// ```
pub struct TriggerRule {
    /// The 16-bit memory address in the NES CPU memory map to inspect.
    pub address: u16,
    /// How many bytes to read starting from the `address` (e.g., `U8` for 1 byte).
    pub width: TriggerWidth,
    /// The mathematical or bitwise comparison to apply between the memory value and the target `value`.
    pub op: TriggerOp,
    /// The target value used as the right-hand side of the comparison operation.
    pub value: u32,
    /// The number of frames the trigger will ignore further matches after successfully triggering. Prevents double-splitting.
    pub debounce_frames: u32,
    /// The number of consecutive frames the condition must be strictly true before the trigger actually fires. Useful for filtering out transient memory states.
...
#[derive(Debug, Clone)]
/// Represents an `RtaProfile` that has been successfully parsed and loaded from disk.
///
/// This binds the logical profile configuration to the specific filesystem path it was loaded from,
/// which is crucial for generating draft reports or overwriting profiles during calibration.
///
/// # Examples
///
/// ```rust
...
///     path: PathBuf::from("config/rta/profiles/smb-any.toml"),
///     profile: RtaProfile::default(),
/// };
/// ```
pub struct LoadedProfile {
    /// The absolute or relative path to the TOML file this profile was loaded from.
    pub path: PathBuf,
    /// The parsed speedrun profile data.
    pub profile: RtaProfile,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Describes the mechanism by which a specific `RtaProfile` was chosen for the current session.
pub enum ProfileSelectionSource {
    /// The profile was automatically chosen because its `rom_hashes` list matched the currently loaded ROM.
    AutoByRomHash,
    /// The user explicitly forced this profile to be used via a command-line argument or UI override.
    ManualOverride,
}

#[derive(Debug, Clone)]
/// The definitive result of the profile selection process, determining which ruleset governs the current run.
///
/// # Examples
///
/// ```rust
...
pub struct ProfileSelection {
    /// The specific profile that has been chosen to govern the session.
    pub selected: LoadedProfile,
    /// The reason why this particular profile was chosen (e.g., auto-detected vs. manually forced).
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Represents the current lifecycle state of a speedrun attempt governed by the `RtaManager`.
pub enum RtaSessionState {
    /// The emulator is running, but the RTA system is not actively tracking a run.
    Idle,
    /// The system is armed and actively scanning memory every frame, waiting for the profile's start condition to be met.
    Armed,
    /// A run is currently in progress. The timer is ticking, and splits are being evaluated.
    Running,
    /// The run has successfully reached its end condition. Artifacts are generated, and tracking stops.
    Finished,
    /// The runner committed a forbidden action (e.g., rewind). The run is dead, but they can continue playing for practice.
    InvalidPractice,
}
...
/// Indicates what triggered a particular split event to occur.
pub enum SplitSource {
    /// The split was triggered by the automated RTA manager detecting a memory state change defined in the profile.
    Automatic,
    /// The split was triggered by the runner pressing a designated manual split hotkey.
    Manual,
}

#[derive(Debug, Clone, Serialize)]
/// A recorded milestone reached during a speedrun attempt.
///
/// # Examples
///
/// ```rust
...
pub struct SplitEvent {
    /// The human-readable name of the split (e.g., `"World 1-1"`).
    pub name: String,
    /// How this split was triggered (automatically via memory inspection, or manually via hotkey).
    pub source: SplitSource,
    /// The absolute emulator frame count at the exact moment the split triggered.
    pub frame: u64,
    /// The elapsed time in milliseconds since the start of the run when this split triggered.
    pub elapsed_ms: u128,
}

#[derive(Debug, Clone, Serialize)]
/// A single frame's worth of controller input data recorded for diagnostic purposes or TAS verification.
///
/// # Examples
///
/// ```rust
...
///     frame: 100,
///     controller1_bits: 0b10000000, // A button pressed
///     controller2_bits: 0,
///     elapsed_ms: 1600,
/// };
/// ```
pub struct InputLogFrame {
    /// The absolute emulator frame number.
    pub frame: u64,
    /// The raw bitmask of buttons pressed on player 1's controller during this frame.
    pub controller1_bits: u8,
    /// The raw bitmask of buttons pressed on player 2's controller during this frame.
    pub controller2_bits: u8,
    /// The elapsed time in milliseconds since the run started at the moment this frame executed.
    pub elapsed_ms: u128,
}

#[derive(Debug, Clone)]
/// Represents an important state change or milestone emitted by the `RtaManager` during a run.
pub enum RtaEvent {
    /// The run has officially begun. The timer is ticking.
    Started,
    /// The run was paused (e.g., due to window focus loss, depending on the profile policy).
    Paused,
    /// The run has resumed from a paused state.
    Resumed,
    /// A new split was reached. Contains the recorded details of the split.
    Split(SplitEvent),
    /// The run was instantly disqualified due to a forbidden action. The string provides the reason.
    Invalidated(String),
    /// The run successfully reached its end condition. Contains the final elapsed time.
    Finished(std::time::Duration),
}

#[derive(Debug, Clone)]
/// Contains the filesystem paths to the verification artifacts generated at the end of a run.
///
/// # Examples
///
/// ```rust
...
///     run_json_path: PathBuf::from("runs/rta/smb_hash.run.json"),
///     input_log_path: None,
/// };
/// ```
pub struct RunArtifactPaths {
    /// The path to the authoritative `.run.json` file containing the split times and validity verdict.
    pub run_json_path: PathBuf,
    /// The path to the frame-by-frame controller input log, if enabled in the profile's `LoggingPolicy`.
...
/// Serde serialization helpers for treating iterators as collections.
pub mod serde_iter {
...
#[derive(Debug, Clone)]
/// The resulting paths generated when running the RTA engine in calibration mode.
///
/// # Examples
///
/// ```rust
/// use nes_desktop::rta::DraftOutput;
/// use std::path::PathBuf;
/// let draft = DraftOutput {
///     profile_path: PathBuf::from("config/rta/profiles/smb.draft.toml"),
///     report_path: PathBuf::from("runs/rta/smb.draft_report.json"),
/// };
/// ```
pub struct DraftOutput {
    /// The path to the new, auto-generated draft `.toml` profile containing the derived trigger rules.
    pub profile_path: PathBuf,
    /// The path to the diagnostic `.json` report explaining how the triggers were calculated.
...
#[derive(Debug, Clone)]
/// An analytical tool used in "calibration mode" to automatically reverse-engineer trigger conditions.
///
/// When a runner manually splits during calibration, this recorder analyzes the memory states leading up
/// to the split to find stable, unique memory addresses that reliably correlate with the milestone. It
/// then writes a draft `RtaProfile` based on these findings.
pub struct CalibrationRecorder {
    profile_id: String,
    frames: std::collections::VecDeque<CalibrationFrame>,
