...
/// The default filesystem path where RTA profiles are stored.
pub const DEFAULT_RTA_PROFILES_DIR: &str = "config/rta/profiles";
/// The default filesystem path where finished RTA run artifacts are saved.
pub const DEFAULT_RTA_RUNS_DIR: &str = "runs/rta";

#[derive(Debug, Clone)]
/// Global configuration for the RTA session.
///
/// Specifies paths and calibration modes.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::RtaRuntimeConfig;
/// use std::path::PathBuf;
/// let config = RtaRuntimeConfig { profile_id_override: None, profiles_dir: PathBuf::from("profiles"), runs_dir: PathBuf::from("runs"), calibrate: false };
/// ```
pub struct RtaRuntimeConfig {
    /// Optionally force a specific profile ID to load, bypassing auto-detection.
    pub profile_id_override: Option<String>,
    /// The directory to search for `.toml` profile definitions.
    pub profiles_dir: PathBuf,
    /// The directory where finished `.run.json` artifacts should be written.
    pub runs_dir: PathBuf,
    /// If true, the session will start in calibration mode to draft a new profile.
    pub calibrate: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The lifecycle status of a speedrun profile.
///
/// ## Examples
/// ```rust
...
pub enum ProfileStatus {
    /// A work-in-progress profile, likely generated by the calibration tool.
    Draft,
    #[default]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The source of truth for the speedrun's stopwatch.
///
/// ## Examples
/// ```rust
...
pub enum TimerClock {
    #[default]
    /// Real-world elapsed wall-clock time.
    Wall,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Determines what the RTA manager should do if the emulator window loses focus.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::FocusLossPolicy;
/// let policy = FocusLossPolicy::Continue;
/// ```
pub enum FocusLossPolicy {
    /// Automatically pause the stopwatch and the game until focus returns.
    AutoPause,
    /// Immediately disqualify the run to prevent foul play.
    Invalidate,
    #[default]
    /// Ignore the focus loss and keep the run active.
    Continue,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default, Hash)]
#[serde(rename_all = "snake_case")]
/// Specific emulator actions that are illegal during an active speedrun.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::ForbiddenAction;
/// let action = ForbiddenAction::Rewind;
/// ```
pub enum ForbiddenAction {
    #[default]
    /// The player attempted to rewind time.
    Rewind,
    /// The player loaded a previously saved state.
    SaveLoad,
    /// The player manually stepped the emulator forward frame-by-frame.
    FrameStep,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The size of the memory value to read for a trigger evaluation.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::TriggerWidth;
/// let width = TriggerWidth::U8;
/// ```
pub enum TriggerWidth {
    #[default]
    /// Read a single byte (8 bits).
    U8,
    /// Read a word (16 bits, little-endian).
    U16,
    /// Read a double word (32 bits, little-endian).
    U32,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The comparison operator used to evaluate a trigger's condition.
///
/// ## Examples
/// ```rust
...
pub enum TriggerOp {
    #[default]
    /// The memory value is exactly equal to the target.
    Eq,
    /// The memory value is not equal to the target.
    Ne,
    /// The memory value is strictly greater than the target.
    Gt,
    /// The memory value is greater than or equal to the target.
    Gte,
    /// The memory value is strictly less than the target.
    Lt,
    /// The memory value is less than or equal to the target.
    Lte,
    /// A specific bitmask is fully set in the memory value.
    BitSet,
    /// A specific bitmask is fully cleared in the memory value.
    BitClear,
    /// The memory value simply changed from its previous state.
    Changed,
}

#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Configures how the run's stopwatch is managed.
///
/// Defines the clock source and what happens when the emulator loses focus.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{TimerPolicy, TimerClock, FocusLossPolicy};
/// let policy = TimerPolicy { clock: TimerClock::Wall, focus_loss: FocusLossPolicy::AutoPause, manual_fallback: false };
/// ```
pub struct TimerPolicy {
    /// The timekeeping mechanism used (e.g., Wall clock vs CPU cycles).
    pub clock: TimerClock,
    /// What the timer should do if the user tabs out of the emulator.
    pub focus_loss: FocusLossPolicy,
    /// Whether manual split triggering is allowed if automatic conditions fail.
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Rules detailing what actions immediately disqualify an active run.
///
/// Violations like using save states or rewinding will trigger an immediate invalidation.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{InvalidationPolicy, ForbiddenAction};
/// let policy = InvalidationPolicy { invalidate_on: vec![ForbiddenAction::Rewind] };
/// ```
pub struct InvalidationPolicy {
    /// A list of forbidden emulator actions that ruin the run's integrity.
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Rules governing how and when splits are recorded during a run.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::SplitPolicy;
/// let policy = SplitPolicy { append_only: true, manual_hotkey: "F9".to_owned() };
/// ```
pub struct SplitPolicy {
    /// If true, splits can only be added to the end, preventing retroactive modification.
    pub append_only: bool,
    /// The keyboard hotkey sequence string used to trigger a split manually.
...
#[serde(default, deny_unknown_fields)]
/// Determines what telemetry and input data is persisted after a run.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::LoggingPolicy;
/// let policy = LoggingPolicy { save_input_log: true };
/// ```
pub struct LoggingPolicy {
    /// Whether to record and save the exact sequence of controller inputs for replay validation.
    pub save_input_log: bool,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(default, deny_unknown_fields)]
/// A specific memory condition that must be met to trigger an event (like starting a run or splitting).
///
/// Constantly evaluated against the emulator's memory space.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{TriggerRule, TriggerWidth, TriggerOp};
/// let trigger = TriggerRule { address: 0x071A, width: TriggerWidth::U8, op: TriggerOp::Eq, value: 1, debounce_frames: 0, require_consecutive: 1 };
/// ```
pub struct TriggerRule {
    /// The exact memory address (e.g., `0x071A`) to inspect.
    pub address: u16,
    /// How many bytes to read starting from the address.
    pub width: TriggerWidth,
    /// The comparison operator (e.g., Equals, GreaterThan) to use against the memory value.
    pub op: TriggerOp,
    /// The target value to compare the memory against.
    pub value: u32,
    /// How many frames must pass before this trigger can be activated again.
    pub debounce_frames: u32,
    /// The number of consecutive frames the condition must remain true to be considered valid.
...
#[derive(Debug, Clone)]
/// A successfully parsed and loaded RTA speedrun profile.
///
/// Contains both the profile rules and the path it was loaded from.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{LoadedProfile, RtaProfile};
/// use std::path::PathBuf;
/// let loaded = LoadedProfile { path: PathBuf::from("profile.toml"), profile: RtaProfile::default() };
/// ```
pub struct LoadedProfile {
    /// The filesystem location where this profile was loaded from.
    pub path: PathBuf,
    /// The actual parsed profile data containing the speedrun rules.
    pub profile: RtaProfile,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Details how a specific RTA profile was selected for use.
///
/// ## Examples
/// ```rust
...
pub enum ProfileSelectionSource {
    /// The profile was automatically selected because its target ROM hash matched the loaded game.
    AutoByRomHash,
    /// The user explicitly forced this profile to load via CLI arguments or configuration.
    ManualOverride,
}

#[derive(Debug, Clone)]
/// Represents the currently active RTA profile and how it was chosen.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{ProfileSelection, ProfileSelectionSource, LoadedProfile, RtaProfile};
/// use std::path::PathBuf;
/// let selection = ProfileSelection { selected: LoadedProfile { path: PathBuf::from("profile.toml"), profile: RtaProfile::default() }, source: ProfileSelectionSource::ManualOverride };
/// ```
pub struct ProfileSelection {
    /// The profile that is currently active.
    pub selected: LoadedProfile,
    /// The method used to select this specific profile.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// The current state of an active Real-Time Attack session.
///
/// Drives the internal state machine of the RTA manager.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::RtaSessionState;
/// let state = RtaSessionState::Running;
/// ```
pub enum RtaSessionState {
    /// The session is inactive and waiting for the game to boot.
    Idle,
    /// The session is ready and actively monitoring memory for the start trigger.
    Armed,
    /// The run is currently in progress.
    Running,
    /// The run has successfully completed all splits.
    Finished,
    /// The run was disqualified due to a rule violation (e.g., rewinding) and is now just practice.
    InvalidPractice,
}
...
/// Describes the origin of a split event during a run.
///
/// ## Examples
/// ```rust
...
pub enum SplitSource {
    /// The split was triggered automatically by a memory condition being met.
    Automatic,
    /// The split was triggered manually by the user pressing a hotkey.
    Manual,
}

#[derive(Debug, Clone, Serialize)]
/// A recorded milestone reached during a speedrun.
///
/// Captures exactly when and how the split occurred.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::{SplitEvent, SplitSource};
/// let event = SplitEvent { name: "Level 1".to_owned(), source: SplitSource::Automatic, frame: 1200, elapsed_ms: 20000 };
/// ```
pub struct SplitEvent {
    pub name: String,
    /// Whether this split was triggered automatically or manually.
    pub source: SplitSource,
    /// The emulator frame number this input corresponds to.
    pub frame: u64,
    /// The elapsed time in milliseconds when this input was registered.
    pub elapsed_ms: u128,
}

#[derive(Debug, Clone, Serialize)]
/// A single frame of controller input recorded for playback and validation.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::InputLogFrame;
/// let log = InputLogFrame { frame: 100, controller1_bits: 0b0000_0001, controller2_bits: 0, elapsed_ms: 1666 };
/// ```
pub struct InputLogFrame {
    /// The emulator frame number this input corresponds to.
    pub frame: u64,
    /// The raw bitmask of buttons pressed on player 1's controller.
    pub controller1_bits: u8,
    /// The raw bitmask of buttons pressed on player 2's controller.
...
#[derive(Debug, Clone)]
/// Notable events emitted by the RTA manager during its lifecycle.
///
/// Used to notify external systems (like UI Overlays) of state changes.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::RtaEvent;
/// let event = RtaEvent::Started;
/// ```
pub enum RtaEvent {
    /// A new run has officially begun.
    Started,
    /// The active run was temporarily paused.
    Paused,
    /// A previously paused run has resumed.
    Resumed,
    /// A milestone split was reached.
    Split(SplitEvent),
    /// The run was disqualified, with a string reason detailing why.
    Invalidated(String),
    /// The run successfully completed in the given total duration.
    Finished(Duration),
}

#[derive(Debug, Clone)]
/// The exact filesystem locations where a finished run's data was saved.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::RunArtifactPaths;
/// use std::path::PathBuf;
/// let paths = RunArtifactPaths { run_json_path: PathBuf::from("run.json"), input_log_path: None };
/// ```
pub struct RunArtifactPaths {
    /// The path to the main `.run.json` artifact containing splits and metadata.
    pub run_json_path: PathBuf,
    /// The path to the optional binary input log file, if logging was enabled.
...
/// Contains utilities to serialize lazily-evaluated iterators.
///
/// Solves the problem of `serde` not natively supporting serialization of non-owned iterators.
///
/// ## Examples
/// ```rust
/// // Used internally as `#[serde(with = "crate::rta::serde_iter")]` on struct fields.
/// ```
pub mod serde_iter {
...
#[derive(Debug, Clone)]
/// The results generated by the auto-calibration drafting tool.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::DraftOutput;
/// use std::path::PathBuf;
/// let output = DraftOutput { profile_path: PathBuf::from("draft.toml"), report_path: PathBuf::from("report.txt") };
/// ```
pub struct DraftOutput {
    /// The path to the generated draft `.toml` profile.
    pub profile_path: PathBuf,
    /// The path to the human-readable analysis report explaining the drafted triggers.
...
#[derive(Debug, Clone)]
/// A high-capacity flight recorder used to analyze memory and auto-draft RTA profiles.
///
/// Observes the user's manual splits and attempts to find matching memory addresses to automate them.
///
/// ## Examples
/// ```rust
/// use nes_desktop::rta::CalibrationRecorder;
/// let recorder = CalibrationRecorder::new("my_profile".to_owned());
/// ```
