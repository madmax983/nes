...
fn format_profile_names<'a>(
    profiles: impl IntoIterator<Item = &'a LoadedProfile> + Clone,
) -> String {
    let iter = profiles.clone().into_iter();
    let len = iter.map(|p| p.profile.id.len() + 2).sum::<usize>();
