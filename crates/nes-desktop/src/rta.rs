...
/// The canonical filesystem location where the emulator searches for [`RtaProfile`] definitions.
/// Speedrunners place their `.toml` files here so the engine knows the rules of engagement.
///
/// # Examples
/// ```
/// use nes_desktop::rta::DEFAULT_RTA_PROFILES_DIR;
/// assert_eq!(DEFAULT_RTA_PROFILES_DIR, "config/rta/profiles");
/// ```
pub const DEFAULT_RTA_PROFILES_DIR: &str = "config/rta/profiles";

/// The designated drop zone for all cryptographically sealed `.run.json` artifacts.
/// When a speedrun concludes, the [`RtaManager`] writes the incontrovertible proof here.
///
/// # Examples
/// ```
/// use nes_desktop::rta::DEFAULT_RTA_RUNS_DIR;
/// assert_eq!(DEFAULT_RTA_RUNS_DIR, "runs/rta");
/// ```
pub const DEFAULT_RTA_RUNS_DIR: &str = "runs/rta";

/// The foundational blueprint for booting up an RTA session.
///
/// This configuration dictates whether we are enforcing a specific rule set
/// via `profile_id_override`, where we read those rules from, and where the
/// resulting evidence (the `.run.json`) should be stored. It bridges the gap
/// between CLI arguments and the strict [`RtaManager`].
///
/// # Examples
/// ```
...
///     profile_id_override: Some("smb_any_percent".to_owned()),
///     profiles_dir: PathBuf::from("config/rta/profiles"),
///     runs_dir: PathBuf::from("runs/rta"),
///     calibrate: false,
/// };
...
pub struct RtaRuntimeConfig {
    /// Forces the [`RtaManager`] to use a specific profile, ignoring the ROM hash match.
    pub profile_id_override: Option<String>,
    /// The filesystem directory where we hunt for `.toml` files to load.
    pub profiles_dir: PathBuf,
    /// The destination folder where the final `.run.json` artifacts are persisted.
    pub runs_dir: PathBuf,
    /// If true, we bypass competitive enforcement and enter a mode to learn memory triggers.
    pub calibrate: bool,
}

/// The seal of approval for an [`RtaProfile`].
///
/// Profiles begin their life as a `Draft` when they are generated by tools like
/// [`CalibrationRecorder`]. Only when a human verifies the memory triggers do they
/// graduate to `Published`. The [`RtaManager`] refuses to load `Draft` profiles
/// during strict competitive play unless explicitly forced.
///
/// # Examples
/// ```
/// use nes_desktop::rta::ProfileStatus;
///
/// let status = ProfileStatus::Published;
/// assert_eq!(status, ProfileStatus::Published);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum ProfileStatus {
    /// A work-in-progress set of rules, usually auto-generated and highly volatile.
    Draft,
    /// A battle-tested rule set approved for tournament use.
    #[default]
    Published,
}

/// Defines the heartbeat of the speedrun timer.
///
/// In competitive speedrunning, time isn't just a number; it's a debate.
/// Do we count wall-clock time (which is vulnerable to lag) or CPU cycles (which
/// require deterministic emulation)? Currently, we rely on the impartial `Wall` clock.
///
/// # Examples
/// ```
/// use nes_desktop::rta::TimerClock;
///
/// let clock = TimerClock::Wall;
/// assert_eq!(clock, TimerClock::Wall);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum TimerClock {
    /// The timer ticks based on the actual passage of time in the real world.
    #[default]
    Wall,
}

/// Determines the penalty when a speedrunner clicks outside the emulator window.
///
/// Pausing a run by clicking another window can be a strategic advantage (to rest)
/// or an accidental tragedy. The [`RtaProfile`] uses this policy to dictate
/// exactly how the [`RtaManager`] should react to a loss of focus.
///
/// # Examples
/// ```
/// use nes_desktop::rta::FocusLossPolicy;
///
/// let policy = FocusLossPolicy::Invalidate;
/// assert_eq!(policy, FocusLossPolicy::Invalidate);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum FocusLossPolicy {
    /// The timer pauses, allowing the runner to take a breath without penalty.
    AutoPause,
    /// The run is immediately destroyed. No mercy for multitasking.
    Invalidate,
    /// The timer marches on. If you aren't looking, you're losing time.
    #[default]
    Continue,
}

/// The cardinal sins of competitive speedrunning.
///
/// The [`RtaManager`] is a strict referee. If a user attempts to use emulator-specific
/// superpowers—like traveling back in time or loading a pristine save state—this enum
/// categorizes the transgression before the run is violently thrown into the
/// [`RtaSessionState::InvalidPractice`] state.
///
/// # Examples
/// ```
/// use nes_desktop::rta::ForbiddenAction;
///
/// let sin = ForbiddenAction::Rewind;
/// assert_eq!(sin, ForbiddenAction::Rewind);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default, Hash)]
#[serde(rename_all = "snake_case")]
pub enum ForbiddenAction {
    /// Attempting to undo a mistake by reversing time.
    #[default]
    Rewind,
    /// Attempting to warp to a previously saved point in reality.
    SaveLoad,
    /// Attempting to play the game one frame at a time for superhuman precision.
    FrameStep,
}
...
/// Defines the size of the memory footprint a [`TriggerRule`] examines.
///
/// Values in the NES's 8-bit architecture are frequently spread across adjacent bytes
/// (e.g., a 16-bit X coordinate). This tells the RTA engine how many bytes to pull
/// and stitch together before running the comparison.
///
/// # Examples
/// ```
/// use nes_desktop::rta::TriggerWidth;
///
/// let width = TriggerWidth::U16;
/// assert_eq!(width, TriggerWidth::U16);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum TriggerWidth {
    /// A single 8-bit byte. The workhorse of NES logic.
    #[default]
    U8,
    /// Two adjacent bytes, read in little-endian order.
    U16,
    /// Four adjacent bytes, read in little-endian order. Rare, but useful for score counters.
    U32,
}

/// The logical assertion evaluated every frame to determine if a speedrun event occurred.
///
/// The [`RtaManager`] reads memory of width [`TriggerWidth`] and executes this operation
/// against the profile's expected value. If it evaluates to `true` (and passes the
/// debounce checks), the trigger fires.
///
/// # Examples
/// ```
/// use nes_desktop::rta::TriggerOp;
///
/// let op = TriggerOp::BitSet;
/// assert_eq!(op, TriggerOp::BitSet);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum TriggerOp {
    /// Triggers when the memory exactly matches the target.
    #[default]
    Eq,
    /// Triggers when the memory differs from the target.
    Ne,
    /// Triggers when the memory exceeds the target.
    Gt,
    /// Triggers when the memory meets or exceeds the target.
    Gte,
    /// Triggers when the memory is less than the target.
    Lt,
    /// Triggers when the memory is less than or equal to the target.
    Lte,
    /// Triggers when a specific bitmask is flipped to `1`.
    BitSet,
    /// Triggers when a specific bitmask is flipped to `0`.
    BitClear,
    /// Triggers when the memory simply mutates, regardless of what it mutates to.
    Changed,
}

/// A contract defining how time flows and falters during an attempt.
///
/// Speedruns exist in a delicate ecosystem. A runner might need to sneeze and click away
/// from the emulator. This policy dictates if the timer pauses for them, or if it marches
/// mercilessly forward, ensuring fairness across the leaderboard.
///
/// # Examples
/// ```
...
///     focus_loss: FocusLossPolicy::AutoPause,
...
pub struct TimerPolicy {
    /// How the stopwatch is driven (e.g., [`TimerClock::Wall`]).
    pub clock: TimerClock,
    /// The strictness applied when the emulator loses operating system focus.
    pub focus_loss: FocusLossPolicy,
    /// Allows the runner to hit a hotkey to start the run if memory detection is flaky.
...
/// The strict blacklist of actions that will instantly murder a speedrun attempt.
///
/// To prevent cheating, the [`RtaManager`] listens for the actions defined here.
/// If a runner performs one (like rewinding), the run transitions into
/// [`RtaSessionState::InvalidPractice`]. The timer keeps running for practice,
/// but the final artifact is branded as invalid.
///
/// # Examples
/// ```
...
pub struct InvalidationPolicy {
    /// The specific actions that will forever taint the current session.
...
/// Governs the mutability and manual interaction of the split timeline.
///
/// Ensures runners can't secretly rewind their splits to hide mistakes.
///
/// # Examples
/// ```
...
pub struct SplitPolicy {
    /// Locks the timeline. Past splits become immutable history.
    pub append_only: bool,
    /// The raw string identifier for the keyboard hotkey used as a manual override.
...
/// Specifies the evidence collected to prove the run's legitimacy.
///
/// Generating gigabytes of input logs is expensive. This policy allows
/// lightweight categories to skip input logging while forcing strict
/// categories to record every button press for anti-cheat verification.
///
/// # Examples
/// ```
...
pub struct LoggingPolicy {
    /// If true, a secondary binary artifact containing frame-by-frame controller bits is saved.
    pub save_input_log: bool,
}

/// The atomic building block of automated refereeing.
///
/// Evaluates a single memory address against a target value. If the logic passes
/// the debounce thresholds, it fires an event (like starting a run or hitting a split).
///
/// # Examples
/// ```
...
pub struct TriggerRule {
    /// The memory offset in CPU RAM to monitor.
    pub address: u16,
    /// Whether to read 1, 2, or 4 bytes from the address.
    pub width: TriggerWidth,
    /// The mathematical comparison used to evaluate the truthiness of the rule.
    pub op: TriggerOp,
    /// The target value to compare the memory against.
    pub value: u32,
    /// Prevents double-firing. The condition must be false for this many frames before it can fire again.
    pub debounce_frames: u32,
    /// Prevents false positives. The condition must remain true for this many continuous frames.
...
/// A named milestone on the speedrun timeline.
///
/// A split typically represents the completion of a level or segment. The engine monitors
/// the underlying [`TriggerRule`]. When it fires, the current stopwatch time is forever
/// etched into the session's `.run.json` artifact under this `name`.
///
/// # Examples
/// ```
/// use nes_desktop::rta::{SplitRule, TriggerRule};
///
/// let rule = SplitRule {
///     name: "Defeat Bowser".to_owned(),
...
pub struct SplitRule {
    /// The human-readable name of the milestone (e.g., `"World 1-1"`).
    pub name: String,
    /// The memory condition that proves the milestone was achieved.
...
/// The master contract for a competitive speedrun category.
///
/// The `RtaProfile` is the constitution. It declares exactly which ROMs are allowed,
/// how the timer functions, what constitutes cheating, and precisely when the timer
/// should start, split, and stop based on RAM manipulation.
///
/// These are typically loaded from TOML files and passed into the [`RtaManager`].
///
/// # Examples
/// ```
...
pub struct RtaProfile {
    /// A unique slug for the profile (e.g., `"smb-any"`). Used as a filename fallback.
    pub id: String,
    /// The human-readable name of the game (e.g., `"Super Mario Bros"`).
    pub game: Option<String>,
    /// The specific leaderboard category (e.g., `"Any%"`).
    pub category: Option<String>,
    /// The semantic version of this rule set. Changes when memory offsets change.
    pub version: Option<String>,
    /// Whether this profile is officially verified for competition.
    pub status: ProfileStatus,
    /// The strict whitelist of SHA-256 hashes for ROMs that are allowed to run this profile.
    pub rom_hashes: Vec<String>,
    /// Defines how the internal stopwatch operates.
    pub timer: TimerPolicy,
    /// Defines which emulator features (like rewinding) immediately disqualify the run.
    pub invalidation: InvalidationPolicy,
    /// Defines how the split timeline can be interacted with.
    pub split_policy: SplitPolicy,
    /// Defines what telemetry is embedded in the final `.run.json` artifact.
    pub logging: LoggingPolicy,
    /// The exact memory condition that causes the stopwatch to begin from 0.
    pub start: TriggerRule,
    /// An optional memory condition that pauses the stopwatch (useful for multi-disc games or known load screens).
    pub pause: Option<TriggerRule>,
    /// An optional memory condition that resumes a paused stopwatch.
    pub resume: Option<TriggerRule>,
    /// The final memory condition that stops the stopwatch and seals the artifact.
    pub end: TriggerRule,
    /// The sequential milestones that the runner must pass during the attempt.
...
/// Binds an [`RtaProfile`] to its origins on disk.
///
/// The engine needs to know where the profile came from so it can re-save drafts
/// or generate artifacts in the correct neighboring directories.
///
/// # Examples
/// ```
/// use std::path::PathBuf;
/// use nes_desktop::rta::{LoadedProfile, RtaProfile};
///
/// let loaded = LoadedProfile {
///     path: PathBuf::from("test.toml"),
///     profile: RtaProfile::default(),
/// };
/// ```
#[derive(Debug, Clone)]
pub struct LoadedProfile {
    /// The absolute path to the `.toml` file that birthed this profile.
    pub path: PathBuf,
    /// The in-memory representation of the rules.
    pub profile: RtaProfile,
}

/// Records the chain of custody for *why* a specific profile was chosen.
///
/// This is embedded into the final artifact so auditors know if a runner bypassed
/// the automatic hash-matching logic by forcing a profile load via CLI flags.
///
/// # Examples
/// ```
/// use nes_desktop::rta::ProfileSelectionSource;
///
/// let source = ProfileSelectionSource::AutoByRomHash;
/// assert_eq!(source, ProfileSelectionSource::AutoByRomHash);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProfileSelectionSource {
    /// The engine autonomously matched the loaded ROM's SHA-256 hash against the profile's whitelist.
    AutoByRomHash,
    /// The runner forcefully injected this profile using `--rta-profile`.
    ManualOverride,
}

/// The culmination of the profile negotiation phase.
///
/// When a ROM is loaded, the engine sifts through all available [`LoadedProfile`]s.
/// If it finds a match (or is forced into one), it yields this struct to the [`RtaManager`]
/// to begin the session.
///
/// # Examples
/// ```
/// use std::path::PathBuf;
/// use nes_desktop::rta::{ProfileSelection, ProfileSelectionSource, LoadedProfile, RtaProfile};
///
/// let selection = ProfileSelection {
///     selected: LoadedProfile {
///         path: PathBuf::from("test.toml"),
///         profile: RtaProfile::default(),
///     },
...
pub struct ProfileSelection {
    /// The verified rule set ready for execution.
    pub selected: LoadedProfile,
    /// The auditable reason this rule set was chosen.
...
/// The state machine of a single speedrun attempt.
///
/// The [`RtaManager`] transitions through these states based on memory triggers
/// and user actions. This enum dictates what UI is shown and whether inputs
/// are currently being logged.
///
/// # Examples
/// ```
/// use nes_desktop::rta::RtaSessionState;
///
/// let state = RtaSessionState::Idle;
/// assert_eq!(state, RtaSessionState::Idle);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RtaSessionState {
    /// The engine is watching memory, waiting for the start trigger.
    Idle,
    /// The start trigger fired, but the manager is waiting for the runner's first input to begin the stopwatch.
    Armed,
    /// The stopwatch is active. The engine is hunting for splits and the end trigger.
    Running,
    /// The run completed successfully. All artifacts are sealed.
    Finished,
    /// The runner committed a [`ForbiddenAction`]. The stopwatch continues for practice, but artifacts are marked invalid.
    InvalidPractice,
}
...
/// Indicates whether a split was triggered by math or a human.
///
/// # Examples
/// ```
/// use nes_desktop::rta::SplitSource;
///
/// let source = SplitSource::Automatic;
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
pub enum SplitSource {
    /// The [`RtaManager`] detected the memory change defined in the [`SplitRule`].
    Automatic,
    /// The runner panicked and hit the manual override hotkey.
    Manual,
}

/// An immutable record of a completed milestone during an active run.
///
/// These are accumulated by the [`RtaManager`] in RAM until the run finishes,
/// at which point they are serialized into the `.run.json` artifact.
///
/// # Examples
/// ```
/// use nes_desktop::rta::{SplitEvent, SplitSource};
///
/// let event = SplitEvent {
///     name: "World 1-1".to_owned(),
///     source: SplitSource::Automatic,
///     frame: 1000,
///     elapsed_ms: 16666,
/// };
/// ```
...
pub struct SplitEvent {
    /// The name of the milestone, inherited from the profile's [`SplitRule`].
    pub name: String,
    /// How the milestone was triggered.
    pub source: SplitSource,
    /// The absolute PPU frame count when the trigger occurred.
    pub frame: u64,
    /// The total stopwatch time (in milliseconds) at the moment of the split.
    pub elapsed_ms: u128,
}

/// A single snapshot of the player's physical input.
///
/// If [`LoggingPolicy::save_input_log`] is enabled, millions of these structs
/// are generated during a run. They allow the run to be perfectly recreated and
/// analyzed for macro abuse or impossibly fast mashing.
///
/// # Examples
/// ```
/// use nes_desktop::rta::InputLogFrame;
///
/// let log = InputLogFrame {
///     frame: 10,
///     controller1_bits: 0x80, // A button
///     controller2_bits: 0x00,
///     elapsed_ms: 166,
/// };
/// ```
#[derive(Debug, Clone, Serialize)]
pub struct InputLogFrame {
    /// The absolute PPU frame count of this snapshot.
    pub frame: u64,
    /// The raw 8-bit latch read of Player 1's controller.
    pub controller1_bits: u8,
    /// The raw 8-bit latch read of Player 2's controller.
    pub controller2_bits: u8,
    /// The exact stopwatch time this input was registered.
    pub elapsed_ms: u128,
}

/// The messaging protocol the [`RtaManager`] uses to talk to the frontend GUI.
///
/// The engine is headless. As it ticks, it yields these events so the desktop
/// application knows when to play a split sound, update the UI timer color,
/// or show an angry red "INVALIDATED" banner.
///
/// # Examples
/// ```
/// use nes_desktop::rta::RtaEvent;
/// use std::time::Duration;
///
/// let event = RtaEvent::Finished(Duration::from_secs(300));
/// ```
#[derive(Debug, Clone)]
pub enum RtaEvent {
    /// The stopwatch has officially started.
    Started,
    /// The stopwatch is frozen (either by memory trigger or focus loss).
    Paused,
    /// The stopwatch is running again.
    Resumed,
    /// A milestone was reached. Time to update the on-screen splits list.
    Split(SplitEvent),
    /// The runner cheated. The string contains the specific [`ForbiddenAction`] reason.
    Invalidated(String),
    /// The run is over. The payload is the final official time.
    Finished(Duration),
}

/// The receipt proving a speedrun was successfully saved to disk.
///
/// Returned by [`RtaManager::write_artifacts_if_finished`] to inform the
/// frontend where the files live so they can be revealed in the OS file explorer.
///
/// # Examples
/// ```
/// use std::path::PathBuf;
/// use nes_desktop::rta::RunArtifactPaths;
///
/// let paths = RunArtifactPaths {
///     run_json_path: PathBuf::from("run.json"),
///     input_log_path: None,
/// };
/// ```
#[derive(Debug, Clone)]
pub struct RunArtifactPaths {
    /// The absolute path to the `.run.json` ledger.
    pub run_json_path: PathBuf,
    /// The absolute path to the binary input log (if recording was enabled).
...
/// Internal helper module for serializing iterators with `serde`.
pub mod serde_iter {
...
/// The result of generating a draft profile from manual calibration.
#[derive(Debug, Clone)]
pub struct DraftOutput {
    /// The path where the generated `.toml` profile was saved.
    pub profile_path: PathBuf,
    /// The path where the human-readable HTML/Markdown report was saved.
...
/// Records a user's manual gameplay and automatically generates an RTA Profile.
///
/// This tool observes memory states when a user hits a hotkey to "split",
/// and statistically attempts to find memory addresses that uniquely identify
/// those events, eliminating the need to manually reverse-engineer RAM addresses.
#[derive(Debug, Clone)]
