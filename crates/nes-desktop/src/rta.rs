...
/// Default directory path for RTA profiles
/// Default directory path for RTA profiles.
///
/// ## Examples
/// ```
...
/// Default directory path for RTA run output files
/// Default directory path for RTA run output files.
///
/// ## Examples
/// ```
...
pub struct RtaRuntimeConfig {
    /// Optional manual override for the profile ID, bypassing auto-selection
    pub profile_id_override: Option<String>,
    /// Directory to search for `.toml` profiles
    pub profiles_dir: PathBuf,
    /// Directory to save `.run.json` artifacts
    pub runs_dir: PathBuf,
    /// If true, runs in calibration mode to learn manual splits
    pub calibrate: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Indicates the maturity and trust level of an RTA profile
/// Indicates the maturity and trust level of an RTA profile.
///
/// ## Examples
/// ```
...
pub enum ProfileStatus {
    /// A draft profile, likely generated by the calibration tool, not yet approved for strict mode
    Draft,
    /// An active, trusted profile used for standard RTA attempts
    #[default]
    Published,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Defines how time is measured during a run
/// Defines how time is measured during a run.
///
/// ## Examples
/// ```
...
/// ```
pub enum TimerClock {
    /// Standard wall-clock time, unaffected by emulator slowdowns
    #[default]
    Wall,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Determines what happens when the emulator loses operating system window focus
/// Determines what happens when the emulator loses operating system window focus.
///
/// ## Examples
/// ```
...
/// assert_eq!(policy, FocusLossPolicy::Continue);
/// ```
pub enum FocusLossPolicy {
    /// Automatically pause the timer (useful for practice, disallowed in strict runs)
    AutoPause,
    /// Instantly invalidate the run if focus is lost (strict tournament mode)
    Invalidate,
    /// Ignore focus loss and keep running (default for most casual setups)
    #[default]
    Continue,
}
...
#[serde(rename_all = "snake_case")]
/// A cardinal sin that immediately invalidates a competitive speedrun attempt
/// A cardinal sin that immediately invalidates a competitive speedrun attempt.
///
/// ## Examples
/// ```
...
/// assert_eq!(action, ForbiddenAction::Rewind);
/// ```
pub enum ForbiddenAction {
    ...
    SaveLoad,
    /// Pausing and stepping frame-by-frame
    FrameStep,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// Defines how many bytes are read from memory for a trigger condition
/// Defines how many bytes are read from memory for a trigger condition.
///
/// ## Examples
/// ```
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize, Default)]
#[serde(rename_all = "snake_case")]
/// The comparison operator used to evaluate a memory condition
/// The comparison operator used to evaluate a memory condition.
///
/// ## Examples
/// ```
...
pub enum TriggerOp {
    ...
    Lte,
    /// The bit at the target index (0-7 for U8) is 1
    BitSet,
    /// The bit at the target index is 0
    BitClear,
...
/// Defines the core timer behaviors and rules for a profile
/// Defines the core timer behaviors and rules for a profile.
///
/// ## Examples
/// ```
...
/// };
/// assert_eq!(policy.clock, TimerClock::Wall);
/// ```
pub struct TimerPolicy {
    /// How time is measured
    pub clock: TimerClock,
    /// How to handle window focus loss
    pub focus_loss: FocusLossPolicy,
    /// Allow manual splits to fallback if automatic memory splits fail
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Defines exactly which emulator features will cause a run to be rejected
/// Defines exactly which emulator features will cause a run to be rejected.
///
/// ## Examples
/// ```
...
///     invalidate_on: vec![ForbiddenAction::Rewind],
/// };
/// assert_eq!(policy.invalidate_on.len(), 1);
/// ```
pub struct InvalidationPolicy {
    /// The specific actions that will trigger an invalidation
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(default, deny_unknown_fields)]
/// Defines how the run's segment splits are processed
/// Defines how the run's segment splits are processed.
///
/// ## Examples
/// ```
...
///     append_only: true,
///     manual_hotkey: "Space".to_string(),
/// };
/// assert!(policy.append_only);
/// ```
pub struct SplitPolicy {
    /// If true, new splits are appended; otherwise they follow a strict pre-defined list
    pub append_only: bool,
    /// The keyboard hotkey used for manual splits
...
#[serde(default, deny_unknown_fields)]
/// Defines what telemetry is saved alongside the final run artifact
/// Defines what telemetry is saved alongside the final run artifact.
///
/// ## Examples
/// ```
...
///     save_input_log: true,
/// };
/// assert!(policy.save_input_log);
/// ```
pub struct LoggingPolicy {
    /// If true, a frame-by-frame log of all controller inputs is embedded in the `.run.json`
    pub save_input_log: bool,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(default, deny_unknown_fields)]
/// A condition that inspects emulator memory to determine if a run milestone has occurred
/// A condition that inspects emulator memory to determine if a run milestone has occurred.
///
/// ## Examples
/// ```
...
/// };
/// assert_eq!(rule.address, 0x071A);
/// ```
pub struct TriggerRule {
    /// The CPU memory address to read (0x0000 - 0xFFFF)
    pub address: u16,
    /// How many bytes to read
    pub width: TriggerWidth,
    /// The comparison to perform against the read memory
    pub op: TriggerOp,
    /// The target value to compare against (or bit index for bitwise ops)
    pub value: u32,
    /// Frames to ignore after a trigger fires before it can fire again
    pub debounce_frames: u32,
    /// Number of consecutive frames the condition must be true before it officially triggers
...
#[derive(Debug, Clone)]
/// Represents a parsed RTA profile and its origin location on disk
/// Represents a parsed RTA profile and its origin location on disk.
///
/// ## Examples
/// ```
...
///     profile,
/// };
/// assert_eq!(loaded.path, PathBuf::from("profile.toml"));
/// ```
pub struct LoadedProfile {
    /// The absolute or relative path to the `.toml` file
    pub path: PathBuf,
...
/// Indicates how the current active profile was chosen
/// Indicates how the current active profile was chosen.
///
/// ## Examples
/// ```
...
///
/// let source = ProfileSelectionSource::ManualOverride;
/// assert_eq!(source, ProfileSelectionSource::ManualOverride);
/// ```
pub enum ProfileSelectionSource {
    /// The profile was automatically selected because its `rom_hash_sha256` matched the loaded ROM
    AutoByRomHash,
    /// The user explicitly requested a specific profile ID via CLI or UI override
    ManualOverride,
}

#[derive(Debug, Clone)]
/// The result of the profile selection process
/// The result of the profile selection process.
///
/// ## Examples
/// ```
...
///     selected: LoadedProfile { path: PathBuf::new(), profile: RtaProfile::default() },
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// The primary state machine for a speedrun attempt
/// The primary state machine for a speedrun attempt.
///
/// ## Examples
/// ```
...
///
/// let state = RtaSessionState::Armed;
/// assert_eq!(state, RtaSessionState::Armed);
/// ```
pub enum RtaSessionState {
    /// No run is currently active or waiting to start
    Idle,
    /// The profile is loaded and waiting for the 'start' trigger condition
    Armed,
    /// The timer is actively running and memory is being monitored for splits
    Running,
    /// The run successfully reached the final condition and an artifact was saved
    Finished,
    /// The runner committed a forbidden action; the run is dead but practice can continue
    InvalidPractice,
}
...
/// Indicates what caused a split event to occur
/// Indicates what caused a split event to occur.
///
/// ## Examples
/// ```
...
///
/// let source = SplitSource::Manual;
/// assert_eq!(source, SplitSource::Manual);
/// ```
pub enum SplitSource {
    /// The split was triggered by a memory condition in the profile
    Automatic,
    /// The user pressed the manual split hotkey
    Manual,
}
...
///     elapsed_ms: 1500,
/// };
/// assert_eq!(event.name, "Level 1");
/// ```
pub struct SplitEvent {
    /// The label for the split (e.g., "Level 1 Complete")
    pub name: String,
    /// Whether it was auto-triggered or manually triggered
    pub source: SplitSource,
    /// The absolute emulator frame count when the split occurred
    pub frame: u64,
    /// The elapsed time since the run started, in milliseconds
    pub elapsed_ms: u128,
}

#[derive(Debug, Clone, Serialize)]
/// A single frame of controller input data, used for later verification or replay
/// A single frame of controller input data, used for later verification or replay.
///
/// ## Examples
/// ```
...
///     frame: 42,
///     controller1_bits: 0b10000000,
///     controller2_bits: 0,
///     elapsed_ms: 700,
/// };
/// assert_eq!(log.frame, 42);
/// ```
pub struct InputLogFrame {
    /// The absolute emulator frame count
    pub frame: u64,
    /// Bitmask of player 1's inputs
    pub controller1_bits: u8,
    /// Bitmask of player 2's inputs
    pub controller2_bits: u8,
    /// The elapsed time since the run started when these inputs were registered
    pub elapsed_ms: u128,
}

#[derive(Debug, Clone)]
/// Events emitted by the `RtaManager` that UIs can use to update displays or play sounds
/// Events emitted by the `RtaManager` that UIs can use to update displays or play sounds.
///
/// ## Examples
/// ```
...
/// assert!(matches!(event, RtaEvent::Started));
/// ```
pub enum RtaEvent {
    /// A new run has begun
    Started,
    /// The timer was paused (e.g., due to focus loss)
    Paused,
    /// The timer has resumed after being paused
    Resumed,
    /// A milestone was reached
    Split(SplitEvent),
    /// The run was immediately disqualified (contains the reason)
    Invalidated(String),
    /// The run successfully completed (contains the final time)
    Finished(Duration),
}

#[derive(Debug, Clone)]
/// The output file paths generated upon completing a run
/// The output file paths generated upon completing a run.
///
/// ## Examples
/// ```
...
///     input_log_path: None,
/// };
/// assert_eq!(paths.run_json_path, PathBuf::from("run.json"));
/// ```
pub struct RunArtifactPaths {
    /// The primary JSON artifact containing splits, times, and signatures
    pub run_json_path: PathBuf,
    /// An optional separate file containing the dense input log
...
/// Internal serialization helpers for handling iterators in serde
/// Internal serialization helpers for iterators
/// Internal serialization helpers for handling iterators in serde.
///
/// ## Examples
/// ```
/// let _ = 1; // This module is used internally by serde to serialize iterators without allocation.
/// ```
pub mod serde_iter {
...
#[derive(Debug, Clone)]
/// Output paths generated by completing a calibration run
/// Output paths generated by completing a calibration run.
///
/// ## Examples
/// ```
...
///     profile_path: PathBuf::from("draft.toml"),
///     report_path: PathBuf::from("report.json"),
/// };
/// assert_eq!(output.profile_path, PathBuf::from("draft.toml"));
/// ```
pub struct DraftOutput {
    /// Path to the generated draft `.toml` profile
    pub profile_path: PathBuf,
    /// Path to the detailed statistical report `.json`
...
#[derive(Debug, Clone)]
/// An automated drafting tool that observes manual gameplay and generates `.toml` profiles.
///
/// Instead of making the user manually inspect memory via a hex editor, this recorder tracks
/// all memory writes during a run. When the user manually splits, the recorder looks for
/// memory addresses that mutated *exactly* at that frame to hypothesize potential triggers.
/// An automated drafting tool that observes manual gameplay and generates `.toml` profiles.
///
/// ## Examples
/// ```
...
///
/// let mut recorder = CalibrationRecorder::new("super_mario".to_string());
/// ```
