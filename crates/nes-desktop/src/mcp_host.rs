...
fn handle_client(mut stream: TcpStream, request_tx: &Sender<ToolRequest>) -> Result<(), String> {
    ...
    while let Some(payload) = read_framed_message(&mut reader, &mut line_buffer)? {
...
/// let result = read_framed_message(&mut reader, &mut String::new()).unwrap();
...
mod tests {
    ...
    fn read_response_with_host_drain(
        ...
        loop {
            host.drain(core);
            match read_framed_message(reader, &mut line_buffer) {
    ...
    fn framed_message_io_round_trips_payload_and_validates_headers() {
        ...
        let payload = read_framed_message(&mut reader, &mut String::new())
        ...
        let err = read_framed_message(&mut bad_reader, &mut String::new())
    ...
    fn host_start_and_client_round_trip_cover_bind_addr_drain_and_dispatch() {
        ...
        let ping_payload = read_framed_message(&mut reader, &mut String::new())
        ...
        let list_payload = read_framed_message(&mut reader, &mut String::new())
...
mod coverage_tests {
    ...
    fn test_mcp_host_payload_size_limit() {
        ...
        let result = read_framed_message(&mut reader, &mut String::new());
        assert_eq!(
            result,
