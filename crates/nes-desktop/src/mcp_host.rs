...
fn handle_client(mut stream: TcpStream, request_tx: &Sender<ToolRequest>) -> Result<(), String> {
    ...
    while read_framed_message(&mut reader, &mut payload)? {
...
/// let mut payload = Vec::new();
/// let success = read_framed_message(&mut reader, &mut payload).unwrap();
/// assert!(success);
...
mod tests {
    ...
    fn read_response_with_host_drain(
        ...
        loop {
            host.drain(core);
            match read_framed_message(reader, &mut payload) {
                Ok(true) => return payload,
    ...
    fn framed_message_io_round_trips_payload_and_validates_headers() {
        ...
        let mut payload = Vec::new();
        let err = read_framed_message(&mut bad_reader, &mut payload)
    ...
    fn host_start_and_client_round_trip_cover_bind_addr_drain_and_dispatch() {
        ...
        assert!(ping_success, "ping payload");
        ...
        assert!(list_success, "tools/list payload");
