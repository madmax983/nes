...
fn bridge_command_tool_name_returns_unsupported() {
    let cmd = nes_web::bridge::BridgeCommand {
        core: nes_core::Command::StepFrame,
    };
...
fn dom_key_maps_all_supported_keys() {
    assert_eq!(
        map_dom_key_to_command("KeyZ", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::A)
    );
    assert_eq!(
        map_dom_key_to_command("KeyX", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::B)
    );
    assert_eq!(
        map_dom_key_to_command("Enter", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Start)
    );
    assert_eq!(
        map_dom_key_to_command("ShiftRight", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Select)
    );
    assert_eq!(
        map_dom_key_to_command("ArrowUp", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Up)
    );
    assert_eq!(
        map_dom_key_to_command("ArrowDown", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Down)
    );
    assert_eq!(
        map_dom_key_to_command("ArrowLeft", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Left)
    );
    assert_eq!(
        map_dom_key_to_command("ArrowRight", true).unwrap().core,
        nes_core::Command::PressButton(nes_core::Button::Right)
    );
}
